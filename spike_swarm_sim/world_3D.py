...
class MultiWorldWrapper:
    def __init__(self, n_cpu, height=1000, width=1000, world_delay=1):
        self.n_cpu = n_cpu
        self._worlds = [World3D(height=1000, width=1000, world_delay=1) for _ in range(n_cpu)]
...
class World3D(object):
    ...
    def add_limiting_walls(self):
        for pos, orient, side in zip ([[11, 0, 1], [-11, 0, 1], [0, 11, 1], [0, -11, 1]],\
            ...
            self.add('wall_side_'+ side, Wall(pos, orient, physics_client=self.physics_client), group='side_wall')
            ...
            if obj['type'] == 'robot':
                ...
                for i, (position, orientation) in enumerate(zip(robot_positions, robot_orientations)):
                    if obj['controller'] is not None:
                        ...
                        controller = None
                    robot = Robot3D(position, orientation, controller=controller, physics_client=self.physics_client, **obj['params'])
    ...
    def connect(self):
        ...
        self.planeId = self.physics_client.loadURDF("plane.urdf", physicsClientId=self.physics_client._client)
        self.gui_params = {}
        if self.render:
        ...
        for obj in self.hierarchy.values():
            obj.add_physics(self.physics_client)
