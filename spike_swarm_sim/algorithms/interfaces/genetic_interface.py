...
class NEATInterface(GeneticInterface):
    ...
    def fromGenotype(self, queries, genotype, min_vals, max_vals):
        ...
        #!* Remove from genotype disabled connections so that they 
        # #* are not transmitted to phenotype. 
        #! for conn in filter(lambda g: g.enabled, effective_genotype.connections):
        #!     effective_genotype.delete_connection(conn.name)
        ...
        for node in effective_genotype.nodes:
            if node.name in self.neural_net.graph['neurons']:
                ...
                continue
            self.neural_net.add_neuron(node.name, node.ensemble, 
                tau=node.tau, bias=node.bias, gain=node.gain, activation=node.activation)
            if node.is_output:
