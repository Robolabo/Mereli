...
import logging
import numpy as np
from .population import BenchmarkNeatPopulation  
...
@algorithm_registry(name='benchmarkNEAT')
class BenchmarkNEAT(EvolutionaryAlgorithm):
    """ 
    """
    def __init__(self, populations, *args, **kwargs):
        populations = {name : BenchmarkNeatPopulation(kwargs['population_size'],\
                pop['min_vals'], pop['max_vals'], pop['objects'], **pop['params'])\
                for name, pop in populations.items()}
        super(BenchmarkNEAT, self).__init__(populations, *args, **kwargs)
