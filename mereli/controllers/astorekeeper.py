...
class LoadBatteryController(RobotController):
    ...
    def step(self, state, reward=0):
        ...
        #Recoger niveles de batería de todos los colores
        bat_levels = self.controller_owner.battery.level
        colors = self.controller_owner.battery_colors
        ...
        for i,color in enumerate(colors):
            bat_lv = bat_levels[i]
            # Dependiendo del color de batería, mira sensor de luz correspondiente
            light_sensor = f'{color}_light_sensor'
            if light_sensor not in state:
                continue  # si no existe ese sensor, saltar
            
            ls_read = state[light_sensor]
