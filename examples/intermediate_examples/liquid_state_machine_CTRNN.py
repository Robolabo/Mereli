...
ann.add_ensemble('H2', 50, tau=100*dt)
ann.set_motor('H1')
...
while not done:
    ...
    for t, (stim1, stim2) in enumerate(zip(stimuli1[D:], stimuli2),start=D):
        ...
        y_true = stimuli1[t-D]
        activities = np.array(ann.step({'I1' : 5*stim1})['A'])
        ...
        beta = iterative_LS(activities, y_true, beta, n, alpha=0.5)
        n += 1
    ...
    # print('TRIAL=',trial)
    if trial % 5 == 0:
        print('Trial=',trial,' MSE=',mse)
        # print('BETA=', beta)
    # if ann.weights.sum()>1000:import pdb; pdb.set_trace()
    trial += 1
    done = trial >= 2000 #or mse < 1e-3
    # import pdb; pdb.set_trace()
    # import pdb; pdb.set_trace()
    # print('W_end ',ann.weights.sum())
# plt.plot(0.5*(stimuli1 + stimuli2))
plt.plot(stimuli1)
# activs = np.array([ann.step({'I1' : stim1, 'I2' : stim2})['A'] for stim1, stim2 in zip(stimuli1,stimuli2)] )
activs = np.array([ann.step({'I1' : 5*stim1})['A'] for stim1, stim2 in zip(stimuli1,stimuli2)] )
